...
export function registerViewRendering(RouterClass: typeof Router): void {
   ...
   */
  RouterClass.prototype.renderView = async function (
    view: string,
    ...
    const viewsConfig = this.config.views || {
      ...
      extensions: ['.html', '.stx'],
      cache: require('node:process').env.NODE_ENV === 'production',
      engine: 'auto',
    }
   ...
   */
  RouterClass.prototype.processTemplate = async function (
    template: string,
    data: Record<string, any>,
   ...
   */
  RouterClass.prototype.view = async function (
    path: string,
