...
if (await realpath('.') !== await realpath(import.meta.dir))
  throw new Error('Run the bun-router build from its package directory')

// Hashed chunks and declarations from removed sources must not enter a later
// package. The whole directory is generated by the two emitters below.
...
if (!build.success)
  throw new AggregateError(build.logs, 'Failed to build bun-router')
