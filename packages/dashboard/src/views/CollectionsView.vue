...
function saveRequest() {
  if (!selectedCollection.value) return
  if (!newRequestData.value.name.trim() || !newRequestData.value.url.trim()) return
...
<template>
  <div class="collections-view">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold">Request Collections</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      >
        <div class="p-4 border-b border-gray-200">
          <h2 class="text-lg font-medium text-gray-900">Your Collections</h2>
          <p class="text-sm text-gray-500">{{ collectionsStore.collections.length }} collections, {{ collectionsStore.totalRequests }} requests</p>
        </div>

        <div v-if="collectionsStore.isLoading" class="p-8 text-center">
          <div class="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-indigo-600 border-r-transparent"></div>
          <p class="mt-2 text-gray-600">Loading collections...</p>
        </div>

        <div v-else-if="filteredCollections.length === 0" class="p-8 text-center">
          <p class="text-gray-600">No collections found. Create your first collection to get started.</p>
        </div>
          ...
          >
            <div class="flex justify-between items-start">
              <div>
                ...
                <p v-if="collection.description" class="text-sm text-gray-500 mt-1 line-clamp-2">{{ collection.description }}</p>
              </div>
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                {{ collection.requests.length }}
              </span>
            </div>
            <p class="text-xs text-gray-500 mt-2">Last updated: {{ new Date(collection.updatedAt).toLocaleString() }}</p>
          </li>
        </ul>
      </div>
      ...
      <div v-if="selectedCollection" class="bg-white rounded-lg shadow overflow-hidden lg:col-span-2">
        <div class="p-4 border-b border-gray-200 flex justify-between items-center">
          <div>
            <div class="flex items-center">
              ...
              <h2 class="text-lg font-medium text-gray-900">{{ selectedCollection.name }}</h2>
            </div>
            <p v-if="selectedCollection.description" class="text-sm text-gray-500 mt-1">{{ selectedCollection.description }}</p>
          </div>
        ...
        <div v-if="selectedCollection.requests.length === 0" class="p-8 text-center">
          <p class="text-gray-600">No requests in this collection. Add your first request to get started.</p>
          <button
            @click="openRequestForm()"
          ...
          <li v-for="request in selectedCollection.requests" :key="request.id" class="p-4 hover:bg-gray-50">
            <div class="flex justify-between items-start">
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-600 font-mono mb-2">{{ request.url }}</p>
      ...
      <div v-else-if="!selectedCollection && !showNewCollectionForm" class="bg-white rounded-lg shadow p-8 text-center lg:col-span-2">
        <div class="text-gray-500">
          <span class="i-carbon-folder text-5xl mb-3"></span>
          <h3 class="text-lg font-medium text-gray-900 mb-1">No Collection Selected</h3>
          <p class="mb-4">Select a collection to view and manage its requests</p>
          <button
            @click="showNewCollectionForm = true"
            class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50"
          >
