...
const cookieEntries = computed(() => {
  if (!request.value || !request.value.cookies) return []
...
<template>
  <div class="request-details-view">
    ...
    <template v-else-if="request">
      ...
      <div class="bg-white rounded-lg shadow p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4">
          ...
          <div>
            <div class="text-sm font-medium text-gray-500 mb-1">Size</div>
            <div>{{ request.size }}</div>
          </div>

          <div class="md:col-span-2">
            <div class="text-sm font-medium text-gray-500 mb-1">URL</div>
            <div class="font-mono text-sm break-all">{{ request.host }}{{ request.path }}</div>
          </div>
      ...
      <div class="bg-white rounded-lg shadow mb-6 overflow-hidden">
        <div class="border-b border-gray-200">
          <nav class="flex -mb-px">
            <button
              ...
              :class="[
                'px-6 py-3 font-medium text-sm border-b-2 whitespace-nowrap',
                activeTab === 'request'
        ...
        <div v-if="activeTab === 'request'" class="p-6">
          <div class="mb-6">
            <h3 class="text-lg font-medium mb-4">Headers</h3>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(header, index) in requestHeaderEntries" :key="index" class="hover:bg-gray-50">
                    <td class="px-6 py-3 text-sm font-medium text-gray-900">{{ header.key }}</td>
                    <td class="px-6 py-3 text-sm text-gray-500 break-all">{{ header.value }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div v-if="request.requestBody">
            <h3 class="text-lg font-medium mb-4">Body</h3>
        ...
        <div v-if="activeTab === 'cookies'" class="p-6">
          <div v-if="cookieEntries.length > 0">
            <h3 class="text-lg font-medium mb-4">Cookies</h3>
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(cookie, index) in cookieEntries" :key="index" class="hover:bg-gray-50">
                    <td class="px-6 py-3 text-sm font-medium text-gray-900">{{ cookie.key }}</td>
                    <td class="px-6 py-3 text-sm text-gray-500 break-all">{{ cookie.value }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <div class="flex space-x-4 mb-6">
        <button class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50">
          <span class="i-carbon-repeat mr-2"></span>
          Replay Request
        </button>
    ...
    <div v-else class="bg-white rounded-lg shadow p-8 text-center">
      ...
      <button @click="goBack" class="mt-4 text-indigo-600 hover:text-indigo-800">
        Back to Requests
      </button>
    </div>
  </div>
